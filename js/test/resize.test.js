...
const resize = (frame, given) => {
  let given = {width: 256, heigth: 256};
  const [base] = Object.entries(given).reduce((current, next) => current[1] > next[1] ? current : next);
  const [more] = Object.entries(given).reduce((current, next) => current[1] < next[1] ? next : current);

  const multiplier = frame[base] / given[base];

  const result = Object.assign({}, {[base]: frame[base], [more]: Math.floor(given[more] * multiplier)});
